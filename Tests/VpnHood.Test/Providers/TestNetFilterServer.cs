...
public class TestNetFilterServer : NetFilter
{
    ...
    private IPAddress[] _additionalBlockedAddresses = [];

    public TestNetFilterServer()
    {
        BlockLoopback = false;
    }
    ...
    public void Init(IPAddress[] blockedAddresses, Tuple<IpProtocol, IPEndPoint, IPEndPoint>[] items)
    {
        NetMap.Clear();
        NetMapR.Clear();
        _additionalBlockedAddresses = blockedAddresses;
    ...
    public override IPEndPoint? ProcessRequest(IpProtocol protocol, IPEndPoint requestEndPoint)
    {
        ...
        if (_additionalBlockedAddresses.Contains(requestEndPoint.Address))
            return null;
