param([Parameter(Mandatory=$true)] [String]$projectDir, [Parameter(Mandatory=$true)] [String]$packageName)

# paths
$credentials = (Get-Content "$PSScriptRoot\..\.user\credentials.json" | Out-String | ConvertFrom-Json)
$versionBase = (Get-Content "$PSScriptRoot\version.json" | Out-String | ConvertFrom-Json)
...
dotnet pack "$projectDir" -c "Release" -o "$publishDir" --runtime any -p:IncludeSymbols=true -p:SymbolPackageFormat=snupkg
