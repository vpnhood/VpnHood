...
public partial class StatisticsView : UserControl, IPage
{
    ...
    private void Fill()
    {
        ...
        var isConnected = VhApp.IsConnected(state);
        ...
        if (isPremiumUser && access != null) {
            ...
            if (VhApp.IsPremiumSupported)
                AddRow(card, s.PremiumBy, VhApp.IsPremiumByAccount ? s.PurchaseSubscription : s.PremiumCode, "active");
        ...
        if (access != null) {
            ...
            if (isConnected) {
                var isUdp = VhApp.IsProtocolEnabled(state, ChannelProtocol.Udp);
