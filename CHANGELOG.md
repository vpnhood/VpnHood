...
* Breaking: Eight NuGet packages are renamed so that no package id starts with an OS: VpnHood.AppLib.Android.Common → VpnHood.AppLib.App.Android, VpnHood.AppLib.Ios.Common → VpnHood.AppLib.App.Ios, VpnHood.AppLib.Linux.Common → VpnHood.AppLib.App.Linux, VpnHood.AppLib.Win.Common → VpnHood.AppLib.App.Windows, VpnHood.AppLib.Android.Ads.AdMob → VpnHood.AppLib.Ads.AdMob.Android, VpnHood.AppLib.Android.GooglePlay → VpnHood.AppLib.Stores.GooglePlay, VpnHood.AppLib.Android.GooglePlay.Core → VpnHood.AppLib.Stores.GooglePlay.Core, VpnHood.AppLib.Ios.AppStore → VpnHood.AppLib.Stores.AppStore. Two more move under the namespace of the kind they implement: VpnHood.Core.Server.Access.FileAccessManager → VpnHood.Core.Server.Access.Managers.FileAccessManagers, VpnHood.Core.IpLocations.SqliteProvider → VpnHood.Net.IpLocations.Providers.SqliteProvider. Each namespace is now its package id: VpnHood.AppLib becomes VpnHood.AppLib.App, VpnHood.AppLib.Droid.Common becomes VpnHood.AppLib.App.Android, VpnHood.Core.Client.Devices becomes VpnHood.Core.Client.Devices.Abstractions, and every *.Droid namespace ends in .Android. A fork swaps the PackageReference and its using directives, must move every VpnHood package to this version at once — an old package beside a new one puts the same types in two assemblies (CS0433) — and must rebuild anything compiled against an old id.
* Breaking: Windows is spelled out in package ids, as Android, Ios and Linux already are and as .NET names the platform (net10.0-windows, OperatingSystem.IsWindows): VpnHood.Core.Client.Devices.Win → VpnHood.Core.Client.Devices.Windows, and VpnHood.AppLib.App.Win (published only as 8.2.852) → VpnHood.AppLib.App.Windows. WinDivert, WinTun and the win-x64 downloads keep their names.
* Breaking: Windows is spelled out in type names too, and every platform's app class is named as VpnHoodAndroidApp and VpnHoodIosApp already are: VpnHoodAppWin → VpnHoodWindowsApp, VpnHoodAppLinux → VpnHoodLinuxApp, WinDeviceUiProvider → WindowsDeviceUiProvider, WinDevice → WindowsDevice, WinVpnService → WindowsVpnService, and WinUiContext → WpfUiContext, named for the WPF window it wraps as AvaloniaUiContext is for Avalonia's. Namespaces are unchanged; a fork renames its references.
* Breaking: The networking libraries with nothing VPN-specific in them form a new VpnHood.Net family, usable by any .NET app: VpnHood.Core.Toolkit, Packets, PacketTransports, TcpStack, TcpStack.Abstractions, IpLocations, the four Quic.* packages and the six VpnAdapters.* packages become VpnHood.Net.*, with the same rest of the name (VpnHood.Core.TcpStack → VpnHood.Net.TcpStack). Their namespaces follow. Three packages that live in their own repos join the family the same way: VpnHood.Core.Proxies → VpnHood.Net.Proxies, VpnHood.Core.IpLocations.Assets.Ip2LocationLite → VpnHood.Net.IpLocations.Assets.Ip2LocationLite, VpnHood.Core.Quic.MsQuic.AndroidNative → VpnHood.Net.Quic.MsQuic.AndroidNative.
* Breaking: AppStoreAppUpdaterProvider and AppStoreInAppUserReviewProvider move from VpnHood.AppLib.App.Ios to the new VpnHood.AppLib.Stores.AppStore.Core (namespace VpnHood.AppLib.Stores.AppStore), the counterpart of VpnHood.AppLib.Stores.GooglePlay.Core: updates and reviews for any App Store app, without the billing and sign-in of VpnHood.AppLib.Stores.AppStore. An iOS head references it to use them.
* Breaking: Every platform starts the app one way. A head hands its platform's Init an AppInitParams — AppId, StorageFolderName, and AppOptionsFactory, which now receives an AppOptionsContext — and the platform calls the factory once it has settled the storage path and the packaged files (AppOptionsContext.StoragePath, PackagedAssetProvider), so a provider made there, such as an account provider, reads the right folder. VpnHoodAndroidApp.Init takes a function that returns them, called only in the app's own process; VpnHoodIosApp.Init takes an IosInitParams, which adds AppGroupId and ProviderBundleId, and builds the IosDevice itself; VpnHoodWindowsApp.Init(AppInitParams, storagePath) and VpnHoodLinuxApp.Init(AppInitParams, storagePath) start the app their service runs, so VpnHoodWindowsApp.Start is gone, as is the Init(AppOptions, args) overload that skipped the platform's defaults. AppOptions takes (AppOptionsContext, isDebugMode), its StorageFolderPath is read-only, and AppOptions.BuildStorageFolderPath is gone. A platform fills DeviceUiProvider, CultureProvider, DeviceId and EventWatcherInterval only where the head left them null; VpnHoodWindowsApp used to replace a head's DeviceUiProvider, and now sets no DeviceId at all: a service's account is the same on every machine, so the app's own ClientId serves.
* Breaking: IosDevice requires the App Group's container path, and VpnHoodIosApp stops at start, naming the App Group, when the group has no container. The fallback to LocalApplicationData is gone: that folder is a different path in the app and in its extension, so the VPN service's folder was never shared and connecting failed later instead. It also requires the name iOS's VPN settings list the configuration under (localizedDescription), which defaulted to "VpnHood"; VpnHoodIosApp passes the app's name.
* Breaking: VpnHoodAppMaui.Init takes the same AppInitParams and starts the app through the platform's Init, defaults included: a MAUI app now gets the device's id as DeviceId on Android and the app's own ClientId on Windows, as every other head does, so its ClientId changes once.
* Breaking: A mobile head derives its classes from its UI's and names only its init params. On Android its Application derives from AndroidAvaloniaApplication (the Avalonia UI) or the new AndroidWebViewApplication (the SPA in a web view) and returns its AppInitParams from CreateInitParams; the base starts the app, hands the UI its API and starts Avalonia, so AndroidAvaloniaUi is gone and the head's own OnCreate is needed only for what must run first. On iOS its AppDelegate derives from IosAvaloniaAppDelegate, in the new VpnHood.AppUi.Hosting.Avalonia.Ios package, or the new IosWebViewAppDelegate, which gives each window scene an IosWebViewSceneDelegate over IosWebViewController, and returns its IosInitParams. AvaloniaUiHosting.PrepareContent gives way to the UI's one start, which every Avalonia host runs: AvaloniaUiHosting.StartAsync, or its halves InitAsync and PrepareAsync, which Android runs from the Application and the activity.
* Breaking: A desktop head names its UI as an IDesktopUi - Run, and BringToFront, which the host asks for the tray's Open and a second launch - from the new VpnHood.AppUi.Hosting.Abstractions, which UI packages and hosts both reference: CliHeadParams.Ui replaces CliHeadParams.RunUi, and the Avalonia UI is AvaloniaDesktopHost of the UI type. LinuxCliHost.Run and CliHost.Run are synchronous and take no token - a Linux head's Main is static int Main - and run the window on the head's main thread and everything else off it. WebViewHost takes the web host it shows the page from, which the platform's view passes in, rather than reading VpnHoodApp.
* Breaking: PortalWebBillingProvider no longer takes an openUrl delegate and opens nothing itself: each plan it lists carries its checkout page (the new SubscriptionPlan.CheckoutUrl), which the UI opens as it opens any link - on a TV, as a QR code for the phone - and its Purchase throws NotSupportedException.
* Breaking: On Linux, CliHeadParams derives from AppInitParams, so AppId is required. IAppDaemonHost is IAsyncDisposable and has no Prepare: VpnHoodLinuxApp.Init takes the single-instance lock and clears what a previous run left by itself, and VpnHoodLinuxApp loses the members of its window days (Run, PrepareAsync, Exit, Exiting, OpenMainWindowRequested, ShowWindowAfterStart).
* Breaking: ClientOptions.AppName, which only named the adapter, is ClientOptions.AdapterName, and ClientOptions.AppId joins it: the adapter carries the app id as VpnAdapterSettings.AppId, which replaces LinuxVpnAdapterSettings.OwnerTag - a Linux tun writes it as its alias, or its first 64 characters, a dash and a hash where an alias cannot hold it as it is, so a later start clears only its own leftover - and LinuxDevice and LinuxVpnService no longer take it. An app names its adapter after its package title (AppOptions.AdapterName, which defaults to the new, required AppOptions.PackageTitle - the build's AppConstants.PackageTitle, which AppFeatures carries too - with _dbg in Debug; only a desktop shows it), so the Windows adapter is VpnHoodClient, no longer "VpnHood! CLIENT"; and a Linux build shows the app's own name, no longer its package title. The server's tun carries com.vpnhood.server (ServerApp.AppId).
* Breaking: On Windows the app runs in a service, as on Linux. A Windows head's Main is static int Main returning WindowsCliHost.Run(args, CliHeadParams), from the new VpnHood.AppUi.Hosting.Cli.Windows: "daemon" is the service (LocalSystem, started with Windows, under the service control manager), the window and its tray run as the signed-in person, not elevated - the heads' manifests are asInvoker - and the other words are the command line, with "service install" and "service uninstall" added. VpnHoodWindowsApp is the service's app: its tray, its single-instance mutex, its command file and their members (OpenMainWindowRequested, ExitRequested, ShowWindowAfterStart, ConnectAfterStart, IsAnotherInstanceRunning) are gone; the tray is WindowsAppTray, over the app's API, and OpenUrlInExternalBrowser and SetWindowTitleBarColor are WindowsShell.OpenUrl and SetTitleBarColor. VpnHoodAppWpf is gone: WpfWebViewUi is the SPA's IDesktopUi, a WPF window over the service.
* Breaking: CliPlatform.ExitUiOnClose gives way to CreateTray - where a tray keeps the UI, closing the window only hides it - and CliPlatform gains InstanceSetup and HostDaemon. A platform's IAppCliPaths writes UiDataPath, under which UiContentCachePath now lies. DesktopUiParams gains UiDataPath and StartHidden.
* Breaking: AppOptions.Ga4MeasurementId no longer defaults to VpnHood's GA4 id: a head that names no id sends no analytics. A fork that relied on the default names its own id.
* Breaking: A head's settings are loaded, not declared: AppConfigsBase and IRequiredAppConfigs are gone. AppConfigs in VpnHood.AppLib.App holds what a product's private appsettings can say - the legal links, the remote settings, the analytics id, endpoint tracking, the endpoint strategy, the default access key, custom data, and UpdateInfoBaseUrl, under which a head names its update feed's file (AppConfigs.GetUpdateInfoUrl) - and AppConfigsLoader.Load fills a product's subclass of it from the appsettings the product's project embeds. What a head is stays in code: the app id, the update feed, the web UI port, the logo, the consent summary and the company are no longer read from appsettings, so a fork that set them there sets them in its head or in its product's options builder.
* Breaking: An app's maker is its build's Company, as its name is VhAppName: VpnHood.AppLib.App's build writes it into AppConstants.CompanyName, which a head hands its AppOptions.CompanyName, and makes the copyright "© Company. All rights reserved." where the app states none. An app that states no Company fails its build, since the SDK would otherwise put its assembly's name there. VpnHood's own apps say OmegaHood LLC, the company the repo already gave every assembly.
* Feature: The build writes AppConstants into an app's product library too - a project of the app that is neither a head nor the iOS extension - with AppName, CompanyName, PackageTitle and IsDebugMode but no ids, which stay each head's. A product's options builder reads them there, so a head passes none (ClientAppOptions.Create(context, appConfigs)). A library of the app that has a class of its own named AppConstants, in no namespace, must rename it.
* Breaking: The two product logos in VpnHood.AppUi.Assets.Classic have brand-free names, as the consent summaries beside them do (privacy-consent-client, privacy-consent-connect): images/VpnHoodClient-logo.png → images/logo-client.png, images/VpnHoodConnect-logo.png → images/logo-connect.png. A head that names one in LogoAssetPath renames it.
* Breaking: AppOptions.AllowEndPointStrategy is on by default, and AppConfigs.AllowEndPointStrategy, also on unless the appsettings turn it off, is what VpnHood's products pass it: every head honours the endpoint strategy a person saved.
* Breaking: AppOptions.AdjustForSystemBars and AppFeatures.AdjustForSystemBars are gone. The native side no longer pads for the system bars and AppState.SystemBarsInfo always reports their size, so a UI keeps clear of them itself, as the Avalonia UI does. A head that set it to false drops the line.
* Breaking: The console and the device log (the system's log: the journal, logcat, os_log, the debugger) write one line per entry, its own line breaks as "\n"; the log file keeps an entry's lines. LogServiceOptions.SingleLineConsole is gone, VhConsoleLogger and VhConsoleLoggerProvider lose their singleLine parameter, TextLogger takes it instead, and VhLogger.CreateConsoleLogger takes none - its flag used to land in includeScopes, so its console now shows the level and scopes as the others do. On Linux the device log writes nothing: .NET sent it to syslog from every run, so the service's journal had each line twice.
* Fix: On Linux each log line appears in the journal once, and whole #linux
* Fix: A head built without the browser page starts and serves its API; only a request for the page fails, naming the missing file. A service that cannot start says why in its log, and on Windows in the Application event log under the service's name.
* Fix: ApiClientBase sends a request body only when there is data. A POST, PUT or PATCH without data used to send the JSON null, and data given with any other method was dropped.
* Feature: A desktop head runs in a debugger without a service: the hidden "dev" command, which the Debug launch profiles pass, holds the app and the window in one process, keeps the app's storage in the person's own folder (IAppCliPaths.DevStoragePath) and runs as whoever started it. CliPlatform gains CreateDevDaemonHost; WindowsDaemonHost and LinuxDaemonHost are made by CreateService or CreateDev. On Linux the instance takes the entry assembly's name, as on Windows, so a run through dotnet is not named "dotnet".
* Breaking: A desktop install's folders are named by the app id, so a Debug build's are never a release's and no two apps share one: on Windows the service's storage under ProgramData and a person's folder under %LOCALAPPDATA% are both named com.vpnhood.client.windows for Client, and on Linux a person's folder under ~/.cache is named com.vpnhood.client.linux. WindowsCliPaths and LinuxCliPaths take the app id. The service, the unit and the command keep the package title, and the folders an earlier release kept are only read, by the one-time import.
* Feature: An app states its identity once - VhAppIdBase and VhAppName, in a Directory.Build.props beside its heads - and VpnHood.AppLib.App's build targets make each head's ids and names from it: ApplicationId (the base and the platform, then .web where the head names that channel in VhAppIdChannel, then .debug in Debug except on iOS), the name each build shows, the executable's Product, and on iOS the bundle's display name, the network extension's bundle id and the App Group in both entitlements. VhAppPackageTitle, taken from the name unless stated, is what a desktop head names its executable and service by, and what every release file's name, the update feeds' included, begins with. The code reads them from AppConstants, which the build writes. What a head states itself is kept, and a project that states no VhAppIdBase is left as it was.
* Update: The app's name on iOS is written as on its other platforms, VpnHood! CONNECT #ios #connect
...
* Update: On Windows, the first time you open this version, your settings, profiles and split lists move into the service, and your device keeps its identity #windows
...
* Fix: On Linux without systemd-resolved, the VPN's DNS server stayed in resolv.conf after disconnecting #linux
* Fix: Stopping the VPN service on Linux while connected now ends the session cleanly before the service exits #linux
* Fix: On Linux, the VPN service's start now clears only its own leftover tunnel, never one another app is using under the same name #linux
* Fix: On Linux, an app name longer than 15 characters or with spaces no longer keeps the VPN from creating its tunnel #linux
