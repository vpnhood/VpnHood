...
public class VpnHoodServer : IAsyncDisposable
{
    ...
    public VpnHoodServer(IAccessManager accessManager, ServerOptions options)
    {
        ...
        ServerHost = new ServerHost(SessionManager, options.DownloadsPath, options.QuicServer);
    ...
    private async Task Configure(CancellationToken cancellationToken)
    {
        try {
            ...
            if (serverConfig.EnableHttp01ChallengeValue)
                _http01ChallengeService.Start(serverConfig.TcpEndPointsValue.Select(x => x.Address).Distinct());
