...
public partial class SplitListInput : UserControl
{
    ...
    private void SetHints(IReadOnlyList<(string Label, string? Sample)> hints)
    {
        ...
        foreach (var (label, sample) in hints) {
            var row = new WrapPanel { Orientation = Avalonia.Layout.Orientation.Horizontal };
            var text = new TextBlock { Text = label, VerticalAlignment = Avalonia.Layout.VerticalAlignment.Center, Margin = new Avalonia.Thickness(0, 0, 6, 0) };
