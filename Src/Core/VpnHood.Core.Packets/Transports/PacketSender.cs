...
public abstract class PacketSender : IPacketSender
{
    ...
    protected abstract ValueTask SendPacketsAsync(List<IpPacket> ipPackets);
    public int QueueLength => _sendChannel.Reader.Count;
    public bool IsSending => _isSending || QueueLength > 0;
    public DateTime LastSentTime { get; private set; } = FastDateTime.Now;
    public bool Disposed { get; private set; }

    protected PacketSender(PacketSenderOptions options, bool singleMode = false)
    {
    ...
    {
        LogPacket(ipPacket, $"Sending a packet to queue by {VhLogger.FormatType(this)}.");
        ...
        // dispose the packet
        LogPacket(ipPacket, $"Dropping packet by {VhLogger.FormatType(this)}. Send queue full.", LogLevel.Debug);
        ...
        while (await _sendChannel.Reader.WaitToReadAsync() && !Disposed) {
            ...
            try {
                ...
                if (ipPackets.Count == 1)
                    LogPacket(ipPackets[0], "Error in sending packet via channel.", exception: ex);
                else
                    VhLogger.Instance.LogError(ex, "Error in sending some packets via channel.");
