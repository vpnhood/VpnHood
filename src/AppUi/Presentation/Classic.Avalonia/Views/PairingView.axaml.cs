...
public partial class PairingView : UserControl, IPage, IDisposable
{
    ...
    {
        try {
            var state = await VhApp.Api.App.StartRemoteAccess(CancellationToken.None);
            Apply(state);
            _timer.Start();
        }
        catch (Exception ex) {
    ...
    public void Dispose()
    {
        ...
        if (_isAlwaysOn)
            return;
        try {
            VhApp.Api.App.StopRemoteAccess(CancellationToken.None).Forget("Could not stop remote access.");
        }
        catch (Exception ex) {
