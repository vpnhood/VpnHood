...
**PLEASE NOTE:** This privacy policy applies to the official "VpnHood client" and official "VpnHood Servers". If you obtain an AccessKey from a third party, you must read the third-party's privacy policy, which is beyond our control.
...
VpnHood client & VpnHood server collect anonymous analytics data, such as technical information, and send them to Google Analytics. This data is entirely anonymous and does not relate to your identity or your device. An example of this data is the number of times the app is run, use of a menu item, operating system version, and device model information. Our open-source code ultimately shows that we don't send any user-identifiable information anywhere.

### Your Client ID

The app identifies itself to analytics and to VpnHood servers with a **Client ID**: a random identifier created inside the app on its first launch. It is not your device's serial number, advertising ID, or any other hardware identifier — nothing about you or your device goes into it. If you delete and reinstall the app, a brand-new Client ID is created, and different VpnHood apps on the same device each have their own unrelated Client ID.

### You can turn analytics off

Analytics is controlled by **Settings → Privacy → "Share anonymous usage data"** in the app. It is on by default; when you turn it off, the analytics component is not merely muted — it is never loaded at all, so nothing is sent to Google Analytics. Turning it off also disables in-app bug-report and feedback sending, since those use the same channel.
...
When you connect to a VpnHood server that we operate, such as our free public servers, we collect information that your VpnHood client sends to us, referred to as Log Data. This Log Data includes the technical information in the previous section, plus:

- The AccessKey your client uses to connect
- We log user IP Addresses and user activity for 30 days
- For user activity, we have to log the time and client endpoint (IP address & Port), with the Client ID, and keep it for 30 days. We do not back up these log files. If the server hosting sends us "Notices of Claimed Infringement," we use these log files to find and notify or suspend the offending access keys.

**Important!** We do not save your destination endpoints such as the domain, URL, or IP address you are browsing.

You should know that we keep user activity just in the VpnHood server log files, not in our database. However, we store technical information associated with the Client ID in our database.
