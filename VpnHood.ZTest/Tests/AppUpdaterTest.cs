...
public class AppUpdaterTest
{
    ...
    public void Download_and_Install()
    {
        ...
        Assert.AreEqual("1.0.1",
            JsonSerializer.Deserialize<PublishInfo>(File.ReadAllText(appFolder.PublishInfoFile))?.Version);
    ...
    public void Install_update_by_fileWatcher()
    {
        ...
        // Check result
        Assert.AreEqual(
            JsonSerializer.Deserialize<PublishInfo>(File.ReadAllText(appFolder.PublishInfoFile))?.Version, "1.0.1");
        Assert.AreEqual("file1-new", File.ReadAllText(Path.Combine(appFolder.Folder, "file1.txt")));
        Assert.AreEqual("file2-new", File.ReadAllText(Path.Combine(appFolder.Folder, "file2.txt")));
    }
    ...
    private class AppFolder
    {
        ...
        public AppFolder(Uri? updateUri = null, string version = "1.0.0", Uri? packageDownloadUrl = null,
            string? packageFileName = null, string content = "old", string? targetFramework = null)
        {
            ...
            PublishInfo = new PublishInfo(version, targetFramework ?? $"net{Environment.Version}",
                "launcher/run.dll")
            {
                UpdateUrl = updateUri?.AbsoluteUri,
                PackageDownloadUrl = packageDownloadUrl?.AbsoluteUri,
                PackageFileName = packageFileName,
                LaunchArguments = new[] {"test", $"-sessionName:{SessionName}"}
            };
