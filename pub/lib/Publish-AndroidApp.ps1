...
# store: google distribution -> google; web and web-arm64 share the web key/id (same as the keystore).
...
#   1. publish.json Distributions.<store>.AndroidKeystoreAlias (non-secret, so it lives in the config),
...
# A fork can therefore drop in a bare keystore (auto-detect) or set AndroidKeystoreAlias in publish.json.
...
if ([string]::IsNullOrWhiteSpace($keystoreAlias)) {
	...
	if ($keyAliases.Count -eq 0) {
		Throw "No PrivateKeyEntry found in '$keystore' (set Distributions.$store.AndroidKeystoreAlias in publish.json)."
	}
	if ($keyAliases.Count -gt 1) {
		Throw "Keystore '$keystore' has multiple key entries ($($keyAliases -join ', ')); set Distributions.$store.AndroidKeystoreAlias in publish.json to name the one to use."
	}
