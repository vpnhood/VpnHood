    ...
    : IRouteMapper
{
    ...
    private static async Task HandleException(HttpContextBase context, Exception ex)
    {
        ...
        else if (ex is HttpRequestException { StatusCode: not null } requestException)
            statusCode = requestException.StatusCode.Value;
    ...
    // The whole API on this transport: one controller per facet, each holding the contract
    // interface it serves. The API itself knows none of this - it is the same object a UI in the
    // app's own process calls directly.
    public ApiRouteMapper AddApi(VpnHoodApi api)
    {
        return AddController(new AppController(api.App))
            ...
            .AddController(new ProxyEndPointController(api.ProxyEndPoints));
    }
}