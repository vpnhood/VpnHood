...
public class BinaryStream : ChunkStream
{
    ...
    private async Task WriteInternalAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
    {
        try
        {
            ...
            BitConverter.GetBytes(count).CopyTo(_writeChunkHeaderBuffer, 0);
            _writeChunkHeaderBuffer[4] = encryptChunk ? (byte)1 : (byte)0;
            await _streamCryptor.WriteAsync(_writeChunkHeaderBuffer, cancellationToken); //header is always encrypted

            // Write the chunk data
            var stream = encryptChunk ? _streamCryptor : SourceStream;
