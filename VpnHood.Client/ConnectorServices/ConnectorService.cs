    ...
    : ConnectorServiceBase(endPointInfo, socketFactory, tcpConnectTimeout, allowTcpReuse)
{
        ...
        where T : SessionResponse
    {
        ...
        await StreamUtil.WriteJsonAsync(mem, request, cancellationToken).ConfigureAwait(false);
        var ret = await SendRequest<T>(mem.ToArray(), request.RequestId, cancellationToken).ConfigureAwait(false);
        ...
        where T : SessionResponse
    {
        ...
        if (clientStream != null)
        {
            ...
            try
            {
                ...
                await clientStream.Stream.WriteAsync((byte[])request.Clone(), cancellationToken).ConfigureAwait(false);
                var response = await ReadSessionResponse<T>(clientStream.Stream, cancellationToken).ConfigureAwait(false);
        ...
        clientStream = await GetTlsConnectionToServer(requestId, cancellationToken).ConfigureAwait(false);

        // send request
        try
        {
            await clientStream.Stream.WriteAsync(request, cancellationToken).ConfigureAwait(false);
            var response2 = await ReadSessionResponse<T>(clientStream.Stream, cancellationToken).ConfigureAwait(false);
            return new ConnectorRequestResult<T>
            {
                Response = response2,
                ClientStream = clientStream
    ...
    {
        var message = await StreamUtil.ReadMessage(stream, cancellationToken).ConfigureAwait(false);
        try
        {
