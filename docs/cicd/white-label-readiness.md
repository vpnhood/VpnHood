...
| 2 | **UI theme** (`UiTheme`) | Hardcoded per app entrypoint: `Connect.Android.Google/App.cs`, `Connect.Android.Web/App.cs`, `Connect.Windows.Web/App.cs`, `Connect.Linux.Web/App.cs`, `Connect.Ios.Apple/AppDelegate.cs` | Selects the colour theme (the logo and the consent summary are `LogoAssetPath` / `PrivacyConsentAssetName`, already read from `appsettings.json`) | Read from `appsettings.json` alongside `AppId` |
