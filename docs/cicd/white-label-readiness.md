...
| 2 | **UI theme** (`UiTheme`) | Hardcoded per app entrypoint: `Connect.Android.Google/App.cs`, `Connect.Android.Web/App.cs`, `Connect.Win.Web/App.cs`, `Connect.Linux.Web/App.cs`, `Connect.Ios/AppDelegate.cs` | Selects the whole visual identity — theme and logo | Read from `appsettings.json` alongside `AppId` |
