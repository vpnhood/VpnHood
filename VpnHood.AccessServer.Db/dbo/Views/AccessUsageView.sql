...
AS
SELECT  T.supportId, --
    dbo.Convert_formatTraffic(T.maxTraffic) AS maxTraffic, T.tokenId, T.tokenName, AU.clientIp, --
    dbo.Convert_formatTraffic(AU.sentTraffic) AS sentTraffic, --
    dbo.Convert_formatTraffic(AU.receivedTraffic) AS receivedTraffic, --
	dbo.Convert_formatTraffic(AU.sentTraffic + AU.receivedTraffic) AS traffic, --
  ...
  FROM  dbo.AccessUsage AS AU
        LEFT JOIN dbo.Token AS T ON T.tokenId = AU.tokenId;
GO
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'AccessUsageView';
