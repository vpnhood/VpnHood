...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    protected async Task<IClientStream> GetTlsConnectionToServer(string streamId, int contentLength, CancellationToken cancellationToken)
    {
        ...
        try {
            ...
            if (EndPointInfo.ProxyClientManager.IsEnabled)
                tcpClient = await EndPointInfo.ProxyClientManager.ConnectAsync(tcpEndPoint, cancellationToken);
            else {
        ...
        SslPolicyErrors sslPolicyErrors)
    {
        ...
        var ret = EndPointInfo.CertificateHash?.SequenceEqual(certificate.GetCertHash()) == true;
        return ret;
    }
