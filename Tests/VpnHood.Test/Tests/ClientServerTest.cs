...
public class ClientServerTest : TestBase
{
    ...
    public async Task UdpPackets_Drop()
    {
        ...
        await using var client = await TestHelper.CreateClient(clientOptions: clientOptions);
        await Assert.ThrowsAsync<OperationCanceledException>(() => TestHelper.Test_Udp(TimeSpan.FromSeconds(3)),
            "UDP must be failed.");
    }
    ...
    public async Task Server_limit_by_Max_TcpChannel()
    {
        ...
        await Task.WhenAll(
            tcpClient1.ConnectAsync(MockIp(TestHelper.WebServer.HttpV4EndPoint1), TestCancellationToken).AsTask(),
            tcpClient2.ConnectAsync(MockIp(TestHelper.WebServer.HttpV4EndPoint1), TestCancellationToken).AsTask(),
            tcpClient3.ConnectAsync(MockIp(TestHelper.WebServer.HttpV4EndPoint2), TestCancellationToken).AsTask(),
            tcpClient4.ConnectAsync(MockIp(TestHelper.WebServer.HttpV4EndPoint2), TestCancellationToken).AsTask());
    ...
    public async Task Set_DnsServer_to_vpnAdapter()
    {
        ...
        Assert.IsTrue(client.SessionInfo?.DnsStatus is { Length: > 0 });
    }
}